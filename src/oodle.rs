...
extern {
  ...
  fn oodle_compress(compressor: i32, level: u64, src: *const u8, srclen: u64, dst: *mut u8) -> i64;
...
impl Oodle {
  ...
  pub fn compress(&self, src: &Vec<u8>, dst: &mut Vec<u8>, compressor: Compressor, level: Level) {
    ...
    let required_len = unsafe { oodle_get_compressed_buffer_size_needed(compressor as u8, src.len() as u64) } as usize;
    dst.resize(required_len, 0);

    let len = unsafe { oodle_compress(compressor as i32, level as u64, src.as_ptr(), src.len() as u64, dst.as_mut_ptr()) } as usize;
    dst.resize(len, 0);
  }
  ...
  pub fn decompress(&self, src: &Vec<u8>, dst: &mut Vec<u8>) {
    ...
    let len = unsafe { oodle_decompress(src.as_ptr(), src.len() as u64, dst.as_mut_ptr(), dst.len() as u64) } as usize;
    if len != dst.len() {
